...
async function writeDemoComponent(tag, node) {
  ...
  await writeFile(path.join(__dirname, `../src/.vuepress/components/${tag}.vue`), componentContent);
}
...
async function writeSidebar(components) {
  ...
  const filePath = path.join(__dirname, '../src/components/component.tags.json');
...
async function writeMarkdown(component) {
  const filePath = path.join(__dirname, `../src/components/${component.tag}.md`);
