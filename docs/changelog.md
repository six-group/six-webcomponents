...
- `six-button`: Loading spinner is no longer oversized — it now scales with the button's font size
  again (regression introduced when `six-spinner` got a fixed default font size).
