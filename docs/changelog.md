...
- Added `filter` and `filterPlaceholder` properties to `six-header-dropdown`.
- Added `caret`, `disabled`, `loading`, `submit` and `reset` properties to `six-header-menu-button`.
- Added `suffix` and `prefix` slots to `six-header-menu-button`.
