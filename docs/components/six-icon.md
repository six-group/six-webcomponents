...
SIX UI Library supports all available material design icons. You can search for icons [here](https://fonts.google.com/icons?selected=Material+Icons).
...
<div style="display: flex; gap: 1em; min-height: 5rem; align-items: center">
  ...
  <six-icon>work</six-icon>
  <six-icon>smartphone</six-icon>
  <six-icon>camera</six-icon>
  <six-icon>local_bar</six-icon>
  <six-icon>local_hotel</six-icon>
  <six-icon>local_phone</six-icon>
  <six-icon>person</six-icon>
  <six-icon>check_box</six-icon>
  <six-icon>train</six-icon>
</div>
```
...
You can choose different executions of the material icons. By default try sticking to the outlined icons.
...
<div style="border: solid 1px #f6f6f6">
  ...
  <div id="icon-showcase" style="max-height: 70vh; overflow: scroll">
    <h1 style="margin-left: 1rem">Outlined</h1>
    <div id="icon-showcase-outlined" style="display: flex; flex-wrap: wrap"></div>
    <h1 style="margin-left: 1rem">Filled</h1>
...
<script type="module">
  ...
  const showcaseFilled = document.getElementById('icon-showcase-filled');
  const showcaseOutlined = document.getElementById('icon-showcase-outlined');
  const getIcons = (filled) =>
    icons
      .split(';')
      .map(
        (i) =>
          `<div data-option="${i}" style="word-break: break-all; margin: 1em; padding: 1em; height: 7.5em; width: 10em; text-align: center; background-color: rgba(164,164,164,0.04);"><six-icon size="xLarge" filled="${filled}">${i}</six-icon><div style="padding: 0.5em;">${i}</div></div>`
      )
      .join('\n');
  showcaseFilled.innerHTML = getIcons(true);
  showcaseOutlined.innerHTML = getIcons(false);

  iconSearch.addEventListener('six-input-input', (event) => {
    const searchForType = (type) =>
      Array.from(document.getElementById(type).children).forEach((n) => {
    ...
    searchForType('icon-showcase-filled');
    searchForType('icon-showcase-outlined');
  });
</script>
```
