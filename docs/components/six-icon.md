...
Enter an icon label and press enter to filter:
...
<script type="module">
  ...
  document.getElementById('icon-showcase-symbols-outlined').innerHTML = getSymbols(false);
  document.getElementById('icon-showcase-symbols-filled').innerHTML = getSymbols(true);
  ...
  iconSearch.addEventListener('six-input-input', () => {
    ...
    filter('icon-showcase-outlined');
    filter('icon-showcase-filled');
    filter('icon-showcase-symbols-outlined');
    filter('icon-showcase-symbols-filled');
  });
</script>
```
